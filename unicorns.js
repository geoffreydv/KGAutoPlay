...
function getPraiseProductionBonus() {
	...
	var rate = getTriValue(byId("praise").parsedValue, 1000);
	...
	var blackObeliskBonus = transcendenceLevel * getMeta(Religion.tu, "blackObelisk").ownedNode.parsedValue * 0.005;
	rate = getHyperbolicEffect(rate, 1000) * (1 + atheismBonus + blackObeliskBonus);
	return rate;
}
...
function calculate() {
	...
	for (var i = unicornBuildings.length - 1; i >= 0; i--) {
		...
		if (bld.name === "unicornPasture") {
			...
			ratioDiff = getHyperbolicEffect(ratioDiff, ratioBase);
